...
#define MAX_FREQ_TIME (1000000.0/XY_FREQUENCY_LIMIT)
   ...
   */
#endif // ADVANCE
#ifdef LASER // Laser intensity modulation
	float newSpindleSpeed ;
	float speedCoef;	
	float laserPart;
	laserPart = laser.intensity / 3 ;
	speedCoef = feed_rate / (block->nominal_speed) ; 
	newSpindleSpeed = laserPart / speedCoef ;
	newSpindleSpeed = ( laserPart * 2 ) + newSpindleSpeed ;
	if (newSpindleSpeed < 0.0){
		newSpindleSpeed = 0.0;
	} else if (newSpindleSpeed > block->laser_intensity){
		newSpindleSpeed = laser.intensity;
	}
	block->laser_intensity = newSpindleSpeed;
